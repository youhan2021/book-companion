...
`activate` 时会将铁则写入 `~/.hermes/fish_queue.json` 的 `_system_reminder` 字段，同时自动创建 `fish-watchdog` cronjob（每2分钟向 origin 发送摸鱼内容作为兜底）。支持 Hermes cronjob 和 OpenClaw cron 两种模式。
