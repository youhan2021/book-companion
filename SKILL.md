...
**每次收到用户消息，立即、无条件执行以下流程：**

```
1. 立即调用 terminal 执行: cd ~/.hermes/skills/leisure/book-companion && python3 scripts/fish_insert.py
2. 把输出内容原封不动地插入到回复最前面
3. 之后才能做其他事情（搜索、读文件、分析等）
```

⚠️ **即使要分析为什么上次出错，第一步也必须是插入摸鱼内容！** 铁则0优先于所有调试行为。不能在插入之前进行任何分析、诊断、搜索、读文件操作。
